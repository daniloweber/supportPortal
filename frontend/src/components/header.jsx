...
const Header = () => {
    ...
    <header style={{ background: '#FDFDFD', display: 'flex', justifyContent: 'space-between', padding: '10px', alignItems: 'center', width: '100%', top: 0, zIndex: 1000 }}>
        ...
        <Link to="/" onClick={handleHomeClick} style={{ color: 'black', textDecoration: 'none', fontSize: '20px', transform: isHomeClicked ? 'scale(0.9)' : 'scale(1)', transition: 'transform 0.25s' }}>Home</Link>
            {isLoggedIn ? <Link to="/TicketView" onClick={handleTicketViewClick} style={{ color: 'black', textDecoration: 'none', fontSize: '20px', transform: isTicketViewClicked ? 'scale(0.9)' : 'scale(1)', transition: 'transform 0.25s' }}>Ticket Übersicht</Link> : null}
            {isLoggedIn ? <Link to="/TicketViewStaff" onClick={handleTicketViewStaffClick} style={{ color: 'black', textDecoration: 'none', fontSize: '20px', transform: isTicketViewStaffClicked ? 'scale(0.9)' : 'scale(1)', transition: 'transform 0.25s' }}>Mitarbeiter Dashboard </Link> : null}
        </div>
